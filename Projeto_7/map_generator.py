...
def points2lines_rec(i_a, i_b, points):
	...
	for i_p, point in enumerate(points):
		if distance2line(i_a, i_b, i_p, points) > threshold:
			lines += points2lines_rec(i_a, i_p, points[0:i_p+1])
			lines += points2lines_rec(0, i_b - i_p, points[i_p:])
...
def main():
	...
	with open(sys.argv[1], 'r') as f:
		for line in f:
			...
			for line in lines:
				print("new Line(" + str(int(line[0][0])+100) + "," + str(int(line[0][1])+100) + "," + str(int(line[1][0])+100) + "," + str(int(line[1][1])+100) + "),")	
